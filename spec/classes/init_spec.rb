      ...
      context "on #{os}" do
        ...
        let(:compliance_file_resource) {
          catalogue.resources.select do |x|
            x.type == 'File' && x[:path] =~ /compliance_report.yaml$/
          end.flatten.first
        }

        let(:report) {
          @report ||= YAML.load_file("#{params['options']['server_report_dir']}/#{facts[:fqdn]}/compliance_report.yaml")
        ...
        context 'when placing the report on the client' do
          ...
          it 'should have a valid YAML report' do
